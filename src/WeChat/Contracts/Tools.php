...
use vitphp\admin\WeChat\Exceptions\LocalCacheException;
...
class Tools
{
    ...
    public static function doRequest($method, $url, $options = [])
    {
        ...
        // 证书文件设置
        if (!empty($options['mch_ssl_cer'])) if (file_exists($options['mch_ssl_cer'])) {
            curl_setopt($curl, CURLOPT_SSLCERTTYPE, 'PEM');
            curl_setopt($curl, CURLOPT_SSLCERT, $options['mch_ssl_cer']);
        } else throw new InvalidArgumentException("Certificate files that do not exist. --- [ssl_cer]");
        // 证书文件设置
        if (!empty($options['mch_ssl_key'])) if (file_exists($options['mch_ssl_key'])) {
            curl_setopt($curl, CURLOPT_SSLKEYTYPE, 'PEM');
            curl_setopt($curl, CURLOPT_SSLKEY, $options['mch_ssl_key']);
