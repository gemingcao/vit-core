...
namespace vitphp\admin\WeChat\Contracts;

use vitphp\admin\WeChat\Exceptions\InvalidArgumentException;
use vitphp\admin\WeChat\Exceptions\InvalidResponseException;
...
class BasicWePay
{
    ...
    protected function callPostApi($url, array $data, $isCert = false, $signType = 'HMAC-SHA256', $needSignType = true)
    {
        ...
        if ($isCert) {
            ...
            $option['mch_ssl_p12'] = $this->config->get('mch_ssl_p12');
            $option['mch_ssl_cer'] = $this->config->get('mch_ssl_cer');

            $option['mch_ssl_key'] = $this->config->get('mch_ssl_key');

            if (is_string($option['mch_ssl_p12']) && file_exists($option['mch_ssl_p12'])) {
                $content = file_get_contents($option['mch_ssl_p12']);
                if (openssl_pkcs12_read($content, $certs, $this->config->get('mch_id'))) {
                    $option['mch_ssl_key'] = Tools::pushFile(md5($certs['pkey']) . '.pem', $certs['pkey']);
                    $option['mch_ssl_cer'] = Tools::pushFile(md5($certs['cert']) . '.pem', $certs['cert']);
                } else throw new InvalidArgumentException("P12 certificate does not match MCH_ID --- ssl_p12");
            }
//            dump($option['mch_ssl_cer']); exit;
            if (empty($option['mch_ssl_cer']) || !file_exists($option['mch_ssl_cer'])) {
                throw new InvalidArgumentException("Missing Config -- ssl_cer", '0');
            }
            if (empty($option['mch_ssl_key']) || !file_exists($option['mch_ssl_key'])) {
