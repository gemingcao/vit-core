...
class BootstrapDetail extends Paginator
{
    ...
    protected  function totalshow()
    {
        ...
        $totalhtml="<li class=\"layui-laypage-prev layui-disabled\"><span  style=\"color: #d2d2d2!important\">共".$this->total."条记录&nbsp&nbsp第".$this->currentPage()."页/共".$this->lastPage()."页</span></li>";
        return $totalhtml;
    ...
    public function render()
    {
        if ($this->hasPages()) {
            if ($this->simple) {
                ...
                return sprintf(
                    '<div class="layui-box layui-laypage layui-laypage-molv"><ul class="pagination"> %s %s %s %s %s %s %s %s </ul></div>',
                    //显示数量页码信息
                    $this->totalshow(),
                    //第一页
                    $this->showfirstpage(),
